...
Static Function TIDevMan()
    ...
    DEFINE WINDOW oSelWnd FROM 0,0 TO 0,0 TITLE "TIDev - Ferramenta de desenvolvimento"
        oSelWnd:oFont := oFont
        oSelWnd:SetColor(CLR_BLACK,CLR_WHITE)
        oSelWnd:nClrText := 0
        oSelWnd:ReadClientCoors()

    ACTIVATE WINDOW oSelWnd MAXIMIZED ON INIT (lRet := SelEmp(@cEmp, @cFil) , oSelWnd:End())
...
Static Function ExportCSV(np, oMsg)
    ...
    //cFile := TFileDialog("Arquivos (*.csv)","Informe o arquivo",0,"C:\",.F.)
...
Static Function ExportXml(np, oMsg)
    ...
    cFile := cGetFile("Arquivos query (*.xml) |*.xml|","Informe o arquivo", 0, "C:\", .F., GETF_LOCALHARD + GETF_LOCALFLOPPY + GETF_NETWORKDRIVE)
    If Empty(cFile)
        Return .F.
    Endif
...
Static Function ExportTab(cAliasDic, np)
    ...
    If cFormato == "Tabela"
        ...
        __Export := cAliasTmp
    ElseIf cFormato == "DTC"
        Dbcreate(cArqTmp, aStruct, "DBFCDX")
        DbUseArea(.T., "DBFCDX", cArqTmp, cAliasTmp,.F.,.F.)
        cComando := "LinDb() "
        __Export := cAliasTmp
    ElseIf cFormato == "CSV"
        __Export := cArqTmp
...
Static Function PrcImpCSV(cAliasTst)
    Local aStru    := (cAliasTst)->(DbStruct())
    Local nFields  := Len(aStru)
    Local aCabCSV  := {}
    Local cLinha   := ""
...
Static Function FolderRPO(oDlg)
    ...
    oPL:Align :=CONTROL_ALIGN_ALLCLIENT
        ...
        oLbx:= TwBrowse():New(01,01,490,490,, aCab,, oPL,,,,,,,,,,,,.F.,,.T.,,.F.,,,)
        oLbx:align := CONTROL_ALIGN_ALLCLIENT
...
Static Function FolderExp(oDlg)
    Local nLin:= 02
    Local nCol:= 02
    Local nWm := __nWidth / 2
    Local nHm := __nHeight / 2
    Local nS1 := 0
    Local oList01
    Local oList02
    Local oGet1
...
Static Function FolderSX6(oFol)
    ...
    oPI:Align :=CONTROL_ALIGN_ALLCLIENT
        ...
        oLbx:= TwBrowse():New(01,01,490,490,,aCab,, oPI,,,,,,,,,,,,.F.,,.T.,,.F.,,,)
        oLbx:align := CONTROL_ALIGN_ALLCLIENT
        oLbx:SetArray(aLista)
        oLbx:bLine   := {|| Retbline(oLbx, aLista ) }
        oLbx:Refresh()
